...
from settings import GRID_SIZE, RL_EPISODES, IL_EPISODES, MAX_STEPS, TIMEOUT, WINDOW_SIZE
...
class RobotSimulatorApp:
    ...
    def setup(self):
        dpg.create_context()
        dpg.create_viewport(
            title="Robot Terrain Learning", 
            width=1920, 
            height=1080,
            decorated=False,  # Убирает рамки и заголовок
            x_pos=0,
            y_pos=0
        )
    ...
    def create_output_window(self):
        with dpg.window(label="Agent Path Visualization", tag=self.output_window_tag,
                       pos=(0, 440), width=380, height=400):
            with dpg.texture_registry():
                dpg.add_raw_texture(
                    width=WINDOW_SIZE, 
                    height=WINDOW_SIZE, 
    ...
    def create_controls_window(self):
        with dpg.window(label="Controls", tag=self.controls_window_tag,
                       pos=(400, 650), width=350, height=190):
    ...
    def create_settings_window(self):
        with dpg.window(label="Settings", tag=self.settings_window_tag,
                       pos=(755, 650), width=350, height=190):
    ...
    def create_results_window(self):
        with dpg.window(label="Results", tag=self.results_window_tag,
                       pos=(1110, 650), width=400, height=190):
    ...
    def reset_layout(self):
        # Reset window positions to new default layout
        dpg.set_item_pos(self.main_window_tag, [20, 40])
        dpg.set_item_pos(self.output_window_tag, [20, 510])
        dpg.set_item_pos(self.controls_window_tag, [490, 40])
        dpg.set_item_pos(self.settings_window_tag, [490, 360])
        dpg.set_item_pos(self.results_window_tag, [890, 40])
        dpg.set_item_pos(self.node_editor_window_tag, [490, 730])
    ...
    def apply_settings(self):
        global GRID_SIZE, RL_EPISODES, IL_EPISODES, MAX_STEPS, TIMEOUT
